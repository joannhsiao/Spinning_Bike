...
public class PlayerController : MonoBehaviour
{
    ...
    private float turnLeftAngle = -130;
    private float turnRightAngle = -50;

    void changeDirection(){
        var y = UnityEditor.TransformUtils.GetInspectorRotation(gameObject.transform).y;
        horizontalInput = Input.GetAxis("Horizontal");
        if (y >= turnLeftAngle && y <= turnRightAngle) {
            Bike.transform.Rotate(Vector3.up, Time.deltaTime * TurnAngle * horizontalInput);
        }
        if (y <= turnLeftAngle && horizontalInput >= 0) {
            Bike.transform.Rotate(Vector3.up, Time.deltaTime * TurnAngle * horizontalInput);
        }
        if (y >= turnRightAngle && horizontalInput <= 0) {
            Bike.transform.Rotate(Vector3.up, Time.deltaTime * TurnAngle * horizontalInput);
        }
    }
    */